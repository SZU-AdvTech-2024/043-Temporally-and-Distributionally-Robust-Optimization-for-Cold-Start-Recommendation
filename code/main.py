...
def init():
    ...
    parser.add_argument('--num_epoch', type=int, default=2, help='Epoch number.')
...
if __name__ == '__main__':
    ...
    pretrained_emb = torch.FloatTensor(pretrained_emb).cpu()
    ...
    ##########################################################################################################################################
    model = CLCRec(warm_item, cold_item, num_user, num_item, reg_weight, dim_E, v_feat, a_feat, t_feat, temp_value, num_neg, contrastive, num_sample).to(device)
    ##########################################################################################################################################
    if args.inference:
        with torch.no_grad():
